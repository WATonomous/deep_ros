...
TEST_CASE_METHOD(deep_ros::test::TestExecutorFixture, "Integration: Complete Inference Pipeline", "[integration][inference]")
{
  ...
  SECTION("Different tensor types work correctly")
  {
    ...
    for (auto dtype : types) {
      ...
      Tensor input(shape, dtype, allocator);
...
TEST_CASE_METHOD(deep_ros::test::TestExecutorFixture, "Integration: Dynamic Reconfiguration Workflows", "[integration][dynamic]")
{
  rclcpp::NodeOptions options;
  options.parameter_overrides({
    {"Backend.plugin", "mock_backend"},
  ...
  SECTION("Runtime model switching")
  {
    ...
    // Load first model
    auto params1 = std::vector<rclcpp::Parameter>{
      rclcpp::Parameter("model_path", "/models/first.onnx")
    };
    ...
    auto allocator = node->get_current_allocator();
    Tensor input1({2, 2}, DataType::FLOAT32, allocator);
    auto output1 = node->run_inference(std::move(input1));
    REQUIRE(output1.size() == 4);
    
    // Switch to second model
    auto params2 = std::vector<rclcpp::Parameter>{
      rclcpp::Parameter("model_path", "/models/second.onnx")
    };
    ...
    Tensor input2({3, 3}, DataType::FLOAT32, allocator);
  ...
  SECTION("Configuration changes during operation")
  {
    ...
    auto params = std::vector<rclcpp::Parameter>{
      rclcpp::Parameter("model_path", "/initial/model.onnx")
    };
    ...
    for (int i = 0; i < 3; ++i) {
      // Run inference
      Tensor input({1, 3}, DataType::FLOAT32, allocator);
      auto output = node->run_inference(std::move(input));
      REQUIRE(output.size() == 3);
      
      // Change configuration
      auto new_params = std::vector<rclcpp::Parameter>{
        rclcpp::Parameter("model_path", "/models/model_" + std::to_string(i) + ".onnx")
      };
...
TEST_CASE_METHOD(deep_ros::test::TestExecutorFixture, "Integration: Error Scenarios and Recovery", "[integration][errors]")
{
  ...
  SECTION("Recovery from plugin loading failure")
  {
    ...
    auto bad_params = std::vector<rclcpp::Parameter>{
      rclcpp::Parameter("Backend.plugin", "nonexistent_backend")
    };
    ...
    auto good_params = std::vector<rclcpp::Parameter>{
      rclcpp::Parameter("Backend.plugin", "mock_backend"),
      rclcpp::Parameter("model_path", "/recovery/model.onnx")
    };
  ...
  SECTION("Lifecycle error recovery")
  {
    ...
    auto params = std::vector<rclcpp::Parameter>{
      rclcpp::Parameter("Backend.plugin", "mock_backend"),
      rclcpp::Parameter("model_path", "/problematic/model.onnx")
    };
    node->set_parameters(params);
    node->configure();
    node->activate();
    ...
    auto good_params = std::vector<rclcpp::Parameter>{
      rclcpp::Parameter("model_path", "/valid/model.onnx")
    };
...
TEST_CASE_METHOD(deep_ros::test::TestExecutorFixture, "Integration: Resource Management", "[integration][resources]")
{
  ...
  SECTION("Memory usage patterns")
  {
    ...
    for (int i = 0; i < 100; ++i) {
      ...
      Tensor input(shape, DataType::UINT8, allocator);
