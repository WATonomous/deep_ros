...
TEST_CASE("Tensor: Basic Construction and Properties", "[tensor][construction]")
{
  SECTION("Default constructor creates empty tensor")
  {
    ...
    REQUIRE(tensor.dtype() == DataType::FLOAT32);  // Should have sensible default
  }
  ...
  SECTION("External data constructor doesn't take ownership")
  {
    ...
    Tensor tensor(data.data(), shape, DataType::FLOAT32);
...
TEST_CASE_METHOD(deep_ros::test::MockBackendFixture, "Tensor: Managed Memory Construction", "[tensor][memory]")
{
  ...
  SECTION("Allocator-based constructor takes ownership")
  {
    ...
    Tensor tensor(shape, DataType::INT32, allocator);
    
    REQUIRE(tensor.shape() == shape);
    REQUIRE(tensor.size() == 12);
    REQUIRE(tensor.byte_size() == 12 * sizeof(int32_t));
    REQUIRE(tensor.data() != nullptr);
    REQUIRE(tensor.dtype() == DataType::INT32);
  ...
  SECTION("Different data types work correctly")
  {
    ...
    Tensor float32_tensor(shape, DataType::FLOAT32, allocator);
    Tensor float64_tensor(shape, DataType::FLOAT64, allocator);
    Tensor uint8_tensor(shape, DataType::UINT8, allocator);
    Tensor bool_tensor(shape, DataType::BOOL, allocator);
...
TEST_CASE("Tensor: Input Validation", "[tensor][validation]")
{
  ...
  SECTION("Empty shape should be rejected")
  {
    ...
    REQUIRE_THROWS_AS(Tensor(data.data(), empty_shape, DataType::FLOAT32), std::invalid_argument);
  }

  SECTION("Zero dimensions should be rejected")
  {
    std::vector<size_t> zero_shape = {2, 0, 3};
    REQUIRE_THROWS_AS(Tensor(data.data(), zero_shape, DataType::FLOAT32), std::invalid_argument);
  }
  ...
  SECTION("Null data pointer should be handled gracefully")
  {
    ...
    REQUIRE_NOTHROW(Tensor(nullptr, shape, DataType::FLOAT32));
  }
}

TEST_CASE_METHOD(deep_ros::test::MockBackendFixture, "Tensor: Memory Management", "[tensor][memory]")
{
  auto allocator = getAllocator();
  ...
  {
    std::vector<size_t> shape = {2, 3};
    Tensor original(shape, DataType::FLOAT32, allocator);
  ...
  SECTION("Move constructor transfers ownership")
  {
    ...
    Tensor original(shape, DataType::UINT8, allocator);
  ...
  {
    std::vector<size_t> shape = {2, 2};
    Tensor source(shape, DataType::INT32, allocator);
...
TEST_CASE_METHOD(deep_ros::test::MockBackendFixture, "Tensor: Shape Operations", "[tensor][shape]")
{
  auto allocator = getAllocator();
  ...
  SECTION("Tensor properties are calculated correctly")
  {
...
TEST_CASE("Tensor: Data Type System", "[tensor][types]")
{
  SECTION("Data type sizes are correct")
  {
    REQUIRE(get_dtype_size(DataType::FLOAT32) == 4);
    REQUIRE(get_dtype_size(DataType::FLOAT64) == 8);
    REQUIRE(get_dtype_size(DataType::INT8) == 1);
    REQUIRE(get_dtype_size(DataType::INT16) == 2);
    REQUIRE(get_dtype_size(DataType::INT32) == 4);
    REQUIRE(get_dtype_size(DataType::INT64) == 8);
    REQUIRE(get_dtype_size(DataType::UINT8) == 1);
    REQUIRE(get_dtype_size(DataType::UINT16) == 2);
    REQUIRE(get_dtype_size(DataType::UINT32) == 4);
    REQUIRE(get_dtype_size(DataType::UINT64) == 8);
    REQUIRE(get_dtype_size(DataType::BOOL) == 1);
  }
...
TEST_CASE_METHOD(deep_ros::test::MockBackendFixture, "Tensor: Edge Cases", "[tensor][edge_cases]")
{
  auto allocator = getAllocator();
