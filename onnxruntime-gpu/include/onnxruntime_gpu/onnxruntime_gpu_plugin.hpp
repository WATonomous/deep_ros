...
/**
 * @brief ONNX Runtime GPU inference plugin
 *
 * Provides GPU-based inference using ONNX Runtime with CUDA memory allocation.
 */
class OnnxRuntimeGpuPlugin : public deep_ros::DeepBackendPlugin
{
public:
  explicit OnnxRuntimeGpuPlugin(int device_id = 0);
  ~OnnxRuntimeGpuPlugin() override = default;
...
private:
  ...
  // TODO(wato): Add ONNX Runtime session and CUDA provider members when implementing inference
};
