...
| `camera_info_topics` | string[] | `[]` | Optional: one `sensor_msgs/CameraInfo` topic per camera (same order). When set, synced camera info is batched and published on `~/multi_camera_info` (same header.stamp as the multi_image for association). |
...
3. **Callback & Publishing**: Creates MultiImage/MultiImageCompressed msgs with timestamp to batch the camera images; optionally publishes batched CameraInfo on `~/multi_camera_info` with the same stamp
