...
"""
Launch test for deep_object_detection with GPU backend (cuda).
...
def generate_test_description():
    ...
    # Deep object detection node with GPU backend
    ...
    def test_gpu_detection_with_dummy_multiimage(self, proc_output):
        ...
        def detection_callback(msg):
            ...
            self.node.get_logger().info(
                f"Received GPU detection output with {len(msg.detections)} detections"
            )
        ...
        self.node.get_logger().info(
            f"GPU detection test passed! Received {len(received_detections[0].detections)} detections"
        )
