...
void ObjectDetectionNode::initializeParameters() {
    // Model parameters
    config_.model_engine_path = this->declare_parameter<std::string>("model_engine_path", "/path/to/model.engine");
    ...
    config_.inference_config.engine_path = config_.model_engine_path;
    ...
    config_.inference_config.use_fp16 = this->declare_parameter<bool>("use_fp16", true);
    ...
    // Load class names
    auto class_names_param = this->declare_parameter<std::vector<std::string>>("class_names", {});
    ...
    RCLCPP_INFO(this->get_logger(), "Initialized with %zu camera topics and %zu classes", 
                config_.camera_topics.size(), config_.inference_config.class_names.size());
}

void ObjectDetectionNode::initializeInference() {
    inference_engine_ = std::make_unique<TensorRTInference>(config_.inference_config);
...
void ObjectDetectionNode::initializeSubscribers() {
    ...
    for (size_t i = 0; i < config_.camera_topics.size(); ++i) {
        auto subscriber = std::make_shared<image_transport::Subscriber>(
            image_transport_->subscribe(
                config_.camera_topics[i], 
                config_.queue_size,
...
void ObjectDetectionNode::initializePublishers() {
    detection_publisher_ = this->create_publisher<vision_msgs::msg::Detection2DArray>(
        config_.detection_topic, config_.queue_size);
    
    if (config_.enable_visualization) {
        visualization_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
            config_.visualization_topic, config_.queue_size);
    }
    
    if (config_.enable_debug) {
        performance_publisher_ = this->create_publisher<std_msgs::msg::String>(
            "/performance_stats", 10);
    }
}
...
void ObjectDetectionNode::setupTimers() {
    ...
    batch_timer_ = this->create_wall_timer(
        timer_period,
        std::bind(&ObjectDetectionNode::batchInferenceCallback, this)
    );
}
...
void ObjectDetectionNode::processBatch() {
    ...
    if (config_.enable_debug) {
        RCLCPP_DEBUG(this->get_logger(), "Batch inference time: %ld ms, batch size: %zu", 
                     inference_time.count(), current_batch_.size());
...
void ObjectDetectionNode::publishDetections(const std::vector<std::vector<Detection>>& batch_detections,
...
void ObjectDetectionNode::publishVisualization(const std::vector<std::vector<Detection>>& batch_detections,
...
visualization_msgs::msg::MarkerArray ObjectDetectionNode::createDetectionMarkers(
    ...
    for (size_t i = 0; i < detections.size(); ++i) {
        ...
        text_marker.text = detection.class_name + " (" + std::to_string(static_cast<int>(detection.confidence * 100)) + "%)";
