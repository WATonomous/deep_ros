...
"""Launch test for deep_sample with CPU backend (onnxruntime_cpu)."""
...
def generate_test_description():
    ...
    config_file = os.path.join(
        get_package_share_directory("deep_sample"), "config", "sample_node_config.yaml"
    )
...
class TestCPUBackend(unittest.TestCase):
    ...
    def test_backend_loads(self, proc_output):
        """Test that CPU backend plugin loads."""
        proc_output.assertWaitFor("Loading plugin: onnxruntime_cpu", timeout=10)
        proc_output.assertWaitFor(
            "Successfully loaded plugin: onnxruntime_cpu", timeout=10
        )
    ...
    def test_node_activates(self, proc_output):
        ...
        proc_output.assertWaitFor(
            "SampleInferenceNode activated with backend: onnxruntime_cpu", timeout=15
        )

    def test_inference_with_dummy_image(self, proc_output):
        """Test end-to-end inference by publishing a dummy image and verifying output."""
        ...
        def output_callback(msg):
            ...
            self.node.get_logger().info(
                f"Received inference output with {len(msg.data)} elements"
            )
        ...
        # Publish dummy image
        self.node.get_logger().info("Publishing dummy image for CPU inference test")
        ...
        self.node.get_logger().info(
            f"CPU inference test passed! Received {len(received_output[0].data)} output values"
        )
