...
public:
    explicit TensorError(const std::string& message) : std::runtime_error(message) {}
};

class Tensor {
public:
    // Factory methods for type safety
    static std::unique_ptr<Tensor> create_float32(
        std::vector<int64_t> shape, 
        std::vector<float> data
    );
    
    static std::unique_ptr<Tensor> create_int32(
        std::vector<int64_t> shape,
    ...
    const T* typed_data() const {
        ...
        if (!validate_type<T>()) {
            throw TensorError(std::format(
                "Type mismatch - requested type doesn't match tensor dtype {}",
...
private:
    ...
    static void validate_shape_and_data(
