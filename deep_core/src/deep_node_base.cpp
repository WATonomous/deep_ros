...
bool DeepNodeBase::load_plugin(const std::string& plugin_name) {
    try {
        ...
        RCLCPP_INFO(get_logger(), "Loaded plugin: %s (backend: %s)", 
                   plugin_name.c_str(), plugin_->backend_name().c_str());
        return true;
...
InferenceResult DeepNodeBase::run_inference(std::vector<std::unique_ptr<Tensor>> inputs) {
    ...
    try {
        ...
        return InferenceResult::error(InferenceError::BACKEND_ERROR, 
